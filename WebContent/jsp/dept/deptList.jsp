...
<script type="text/javascript">
	function delDept(obj){
		if(confirm("确定删除此部门?"))
			location.href="<%=path%>/deptCtrl/deleteDept.do?id="+obj;
 	}
	...
	function sum(totalPage){
		...
		location="<%=path%>/deptCtrl/deptInfo.do?currentPage="+jumpPage;
	}
</script>
</head>
<body>
				...
				<div id="pageDiv">
					...
					<c:choose>
						<c:when test="${pageData.currentPage!=1}">
							<a href="<%=path%>/deptCtrl/deptInfo.do?currentPage=${pageData.currentPage-1}"><input
					...
					<c:forEach items="${pageData.itemList }" var="p">
						<c:choose>
							<c:when test="${pageData.currentPage==p}">
								<!-- 突出显示当前页 -->
								<a href="<%=path%>/deptCtrl/deptInfo.do?currentPage=${p}" class="currentPage">${p }</a>
							</c:when>
							<c:otherwise>
								<a href="<%=path%>/deptCtrl/deptInfo.do?currentPage=${p}">${p }</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<!-- 下一页 -->
