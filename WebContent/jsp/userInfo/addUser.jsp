...
<script type="text/javascript" src="<%=path %>/js/jQuery/jquery-3.4.1.min.js"></script>
...
<script type="text/javascript">
	...
	$(function() {
		$("#addUser").click(function() {
			...
			$.post("<%=path %>/user/ajaxReturn.do?userNo=" + v1, function(msg) {
				...
				if (returnData != '' && returnData != null) {//根据传到后台的用户名判断返回不为空即已存在此用户名
					...
					addUserForm.action = "<%=path %>/user/addUser.do?deptName=" + v5+"&type=1";
					addUserForm.submit();
				}
			}, "json");
		});
