...
<script type="text/javascript">
	...
	function delUser(obj1,obj2){//注销用户
		if(confirm("确定注销该用户吗?")){
			...
			location.href="<%=path%>/user/deleteUserVirtual.do?userID="+obj1+"&type=1";			
		}
	}
	
	function active(userID){//激活用户
		if(confirm("确定激活该用户吗?")){
			location="<%=path%>/user/activeUser.do?userID="+userID+"&type=1";
		}
	}
	
	function del_Complete(userID){//彻底删除
		if(confirm("删除将无法恢复,确定继续执行吗?")){
			location="<%=path%>/user/deleteUserCompletely.do?id="+userID+"&type=0";
		}	
	}
	...
	function sum(totalPage,type){//页码跳转
		...
		location="<%=path%>/user/userList.do?currentPage="+jumpPage+"&type="+type;
	}
	...
	function setSelect(){//设置下拉列表
		...
		status.innerHTML="";
		if(type==0)
			status.innerHTML="<option name='op2' value='cancel'>注销用户</option><option name='op1' value='normal'>正常用户</option>";
		else
			status.innerHTML="<option name='op1' value='normal'>正常用户</option><option name='op2' value='cancel'>注销用户</option>"	;
	}
	
	function show(){//显示可选项
		var selectedVal=$("#normalOrCancel").val();
		if(selectedVal=="normal")	
			location.href="<%=path%>/user/userList.do?type=1";			
		else
			location.href="<%=path%>/user/userList.do?type=0";	
	}
</script>
</head>
			...
			<c:forEach items="${userList}" var="list" varStatus="st">
				<tr>
					...
					<td nowrap><c:if test="${USERSESSION.av.authVal==1}">
							<a href="<%=path%>/user/toUpdateUser.do?userNo=${list.userNo}"
		...
		<div id="pageDiv">
			...
			<c:choose>
				<c:when test="${pageData.currentPage!=1}">
					<a
						href="<%=path%>/user/userList.do?currentPage=${pageData.currentPage-1}&nextOrPre=previous&start=${pageData.start}&end=${pageData.end}"><input
			...
			<c:forEach items="${pageData.itemList}" var="p" varStatus="vs">
				...
				<c:choose>
					...
					<c:otherwise>
						<a href="<%=path%>/user/userList.do?currentPage=${p}">${p}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<!-- 下一页 -->
			<c:choose>
				<c:when test="${pageData.currentPage < pageData.totalPage }">
					...
					<a
						href="<%=path%>/user/userList.do?currentPage=${pageData.currentPage+1}&nextOrPre=next&start=${pageData.start}&end=${pageData.end}"><input
