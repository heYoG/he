...
function sum(totalPage){//页码跳转
	...
	location="<%=path%>/user/approveUser.do?currentPage="+jumpPage;
}
...
function refuse(userID){//审批不通过
	if(confirm("拒绝通过审批?"))
		location="<%=path%>/user/refuseUser.do?userID="+userID+"&type=3";
}
</script>
</head>
<body>
					...
					<c:forEach items="${userList}" var="list" varStatus="st">
						<tr>
							...
							<td nowrap><c:if
								...
								</c:if> <c:if test="${list.status==3 and USERSESSION.av.authVal==1}">
									<a
										href="<%=path%>/user/toUpdateUser.do?userNo=${list.userNo}&isAppro=3">修改</a>
								</c:if><c:if test="${list.status==3 and USERSESSION.av.authVal!=1}">
									<input type="button" value="不可操作" disabled="true" title="普通用户不可操作">
								</c:if></td>
						</tr>
					</c:forEach>
				</table>
				...
				<div id="pageDiv">
					...
					<c:choose>
						<c:when test="${pageData.currentPage!=1}">
							<a
								href="<%=path%>/user/approveUser.do?currentPage=${pageData.currentPage-1}&nextOrPre=previous&start=${pageData.start}&end=${pageData.end}"><input
					...
					<c:forEach items="${pageData.itemList}" var="p" varStatus="vs">
						...
						<c:choose>
							...
							<c:otherwise>
								<a href="<%=path%>/user/approveUser.do?currentPage=${p}">${p}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<!-- 下一页 -->
					<c:choose>
						<c:when test="${pageData.currentPage!=pageData.totalPage }">
							<!-- 即不为最后一页 -->
							<a
								href="<%=path%>/user/approveUser.do?currentPage=${pageData.currentPage+1}&nextOrPre=next&start=${pageData.start}&end=${pageData.end}"><input
