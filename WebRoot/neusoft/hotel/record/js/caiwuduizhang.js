...
var cdt_f={};
var reg="";//判断
var thisUrl=baseUrl(3)+"Seal/neusoft/hotel/record/caiwuduizhang.jsp";
var bs;
...
function getDatas(){
	dwr.engine.setAsync(false); //设置方法调用是否同步，false表示同步
	HotelRecordRoomMerge.getRecordRoomMergeList(page,pageSize,cdt_f,cb_data);
}
...
function toArray(d){
	var row=new Array();
	var fileName = toRocordStr(d.cid ,d.filename, d.savefilename);
	row.push(fileName);
	row.push(d.guestname);
	row.push(d.roomno);
	row.push(d.mergedate);
//	row.push(d.mtplname);
	row.push(d.createtime.Format("yyyy-MM-dd hh:mm:ss"));
//	row.push(d.cip);
//	var oprStr=toOprStr(d.cid,d.cfilefilename,d.cdata);
//	row.push(oprStr);
...
function toOprStr(id,name,cdata){
	var str="";
	str+="<input type='button' onclick=\"jiherecord('";
	str+=id+"','"+cdata;
	str+="');flag=true;\" class='SmallButton' value='稽核' />";
	return str;
}
