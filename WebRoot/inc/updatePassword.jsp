...
<link rel="stylesheet" type="text/css"
	href="theme/${sysuser.user_theme}/style.css">
...
<script type="text/javascript">
	...
	function dosubmit() {
		...
		var user_psd1 = document.form1.originalPassword.value;
		dwr.engine.setAsync(false);//必须同步，否则无法获取MD5返回值
		var MD5psd1 = MD5Util.MD5(user_psd1);//MD5加密输入的原始密码
		var user_psd2 = document.getElementById("user_psd").value;
		dwr.engine.setAsync(false);
		var MD5psd2=MD5Util.MD5(user_psd2);//MD5加密新密码
		...
		if ((user_psd2 != "") && (user_psd3 != "")) {
			if (user_psd2 == userId) {
				alert("密码和用户名相同，请重新输入！");
				return false;
			}else if(MD5psd2=="${sysuser.password1md5}"||MD5psd2=="${sysuser.password2md5}"||MD5psd2=="${sysuser.user_psd}"){
				alert("新密码与最近使用过的三次密码相同,请重新输入!");
				return false;
			} else if (!user_psd2.match(expr)) {
				alert("新的密码必须是字母、数字和特殊字符的组合且至少6位长度");
				return false;
			} else if (user_psd2 != user_psd3) {
		...
		for (i = 0; i < myObj.length; i++) {
			if (myObj[i].checked) {
				if (myObj[i].value == "1" || myObj[i].value == "2"
						|| myObj[i].value == "3") {
					if (document.form1.begin_time.value == ""
		...
		LogSys.logAdd("${current_user.user_id}", "${current_user.user_name}",
				"${user_ip}", "初始密码修改", "修改用户:'"
						+ document.form1.user_name.value + "'密码成功", call);//logSys.js
		...
		for (i = 0; i < myObj.length; i++) {
			if (myObj[i].checked) {
				...
				if (myObj[i].value == "3") {
					...
					if (document.form1.key_sn.value != null
							&& document.form1.key_sn.value != "") {
						//SysUser.getUserIdBy_key(obj.SerialNumber,callback1);
						SysUser.getUserBy_keySN(document.form1.key_sn.value,
								callback1);
					}
					...
					if ((obj.SerialNumber == "") || (obj.SerialNumber == null)) {
						...
						document.form1.begin_time.value = obj
								.GetCertStartTimeEx(0);
						document.form1.end_time.value = obj.GetCertEndTimeEx(0);
						if (document.form1.key_sn.value != null
								&& document.form1.key_sn.value != "") {
							//SysUser.getUserIdBy_key(obj.SerialNumber,callback1);
							SysUser.getUserBy_keySN(
									document.form1.key_sn.value, callback1);
						}
					}
				}
			}
	...
	function clearData() {
		...
		document.form1.pfxPsw.value = "";
		document.getElementById("fileDiv").innerHTML = document
				.getElementById("fileDiv").innerHTML;
	}
	function getPfxInfo() {
		var temp = document.getElementById("pfxPath");
		temp.select();
	...
	function getCertInfo2() {
		...
		document.form1.key_cert.value = obj.GetCardCert();
		alert(document.form1.key_cert.value);
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="3"
		...
		<tr>
			<td class="Big">&nbsp;<img src="images/menu/comm.gif"
				align="absmiddle"> <span class="big3"> 用户更改密码 </span>
			</td>
		</tr>
	</table>
	<form action="updateSysUser.do?is_app=updatePsd" method="post"
		...
		<table class="TableBlock" width="95%" align="center">
			...
			<tr style="display:none">
				...
				<td nowrap class="TableData"><label for="USEING_KEY">
					...
					onclick="hiddenZS('3')" />IE证书 <input id="isCer" name="useing_key"
					type="radio" value="4" onclick="hiddenZS('4')" />Cert</td>
			</tr>
			<tr>
			<tr id="zhsh">
			...
			<tr id="zhsh4" style="display:none">
				<td nowrap class="TableData">cert路径：</td>
				<td nowrap class="TableData">
					<div id="fileDiv">
						<input type="file" name="certContent" id="certPath"> <input
							type="button" class="SmallButton" value="获取证书信息"
							onclick="getCertInfo();">
					</div></td>
			</tr>
			...
			<tr>
				...
				<td nowrap class="TableContent"><input type="text"
					...
					value="${sysuser.user_id }" > &nbsp; <span
					id="user_id_msg"></span></td>
			</tr>
			<tr>
				<td nowrap class="TableContent">姓名：</td>
				<td nowrap class="TableContent"><input type="text"
					name="user_name" class="BigInput" size="10"
					value="${sysuser.user_name }" maxlength="30" readonly>
					&nbsp;</td>
			</tr>
			<tr>
				<td nowrap class="TableContent">原始密码：</td>
				<td nowrap class="TableContent"><input type="Password"
					name="originalPassword" id="oriPassword" class="BigInput" size="10"
					maxlength="15"> &nbsp;</td>
			</tr>
			<tr>
				<td nowrap class="TableContent">新的密码：</td>
				<td nowrap class="TableContent"><input type="password"
					name="user_psd" id="user_psd" class="BigInput" size="10"
					maxlength="15"> &nbsp;</td>
			</tr>
			<tr>
				<td nowrap class="TableContent">确认密码：</td>
				<td nowrap class="TableContent"><input type="password"
					name="user_psd2" id="user_psd2" class="BigInput" size="10"
					maxlength="15"> &nbsp;</td>
			</tr>
			<tr>
				...
				<td nowrap class="TableContent" id="sel_priv"><textarea cols=30
					...
					onClick="return openmodwin();" title="选择角色" disabled="disabled" />
				</td>
			</tr>
			<tr>
				<td nowrap class="TableData">部门：</td>
				<td class="TableData"><input type="text" name="dept_name"
					readonly="readonly" value="<%=unitname%>" readonly /> <input
					type="button" value="选 择" class="SmallButton"
					onclick="return openmodwin2();" title="选择部门" disabled="disabled">
				</td>
			</tr>
				...
				<td nowrap class="TableData"><select name="rang_type"
					class="BigSelect" OnChange="select_dept()" >
						...
						<input type="checkbox" name="is_junior2" checked
							onclick="chkbox(this);" id="is_junior2" disabled="disabled">
			...
			<tr>
				...
				<td nowrap class="TableData"><textarea name="user_remark2"
						id="user_remark2" class="BigInput" cols="50" rows="2"
						readonly>${sysuser.user_remark}</textarea>
				</td>
			</tr>
			<tr
			...
			<tbody id="option2" style="display: none;">
				<tr>
					<td nowrap class="TableData">性别： <br> <br> <br>
						<br>
					</td>
				...
				<tr>
					<td nowrap class="TableData">生日： <br> <br> <br>
						<br>
					</td>
					<td nowrap class="TableData"><input type="text"
						name="user_birth" size="10" maxlength="10"
						value="${sysuser.user_birth }" class="BigInput"
						onfocus="this.blur()" readonly><br> <br>
						<br> <br>
					</td>
				</tr>
				<tr>
					<td nowrap class="TableData">界面主题： <br> <br> <br>
						<br>
					</td>
					<td class="TableData"><select name="user_theme"
						class="BigSelect" >
							<c:if test="${sysuser.user_theme==1}">
								<option value="1" >默认主题</option>
							</c:if>
							<c:if test="${sysuser.user_theme==2}">
								<option value="2">清新怡然</option>
							</c:if>
							<c:if test="${sysuser.user_theme==3}">
								<option value="3">简洁明亮</option>
							</c:if>
							<c:if test="${sysuser.user_theme==4}">
								<option value="4">粉红浪漫</option>
							</c:if>
							<c:if test="${sysuser.user_theme==5}">
				...
				<tr>
					<td nowrap class="TableData">手机： <br> <br> <br>
						<br>
					</td>
					<td class="TableData"><input type="text" name="mobil_no"
						size="16" maxlength="23" class="BigInput"
						value="${sysuser.mobil_no }" readonly> <br>
						<br> <br> <br> <br>
					</td>
				</tr>
				<tr>
					<td nowrap class="TableData">电子邮件： <br> <br> <br>
						<br>
					</td>
					<td class="TableData"><input type="text" name="user_email"
						size="25" maxlength="50" class="BigInput"
						value="${sysuser.user_email }" readonly> <br>
						<br> <br> <br>
					</td>
				</tr>
				<tr>
					<td nowrap class="TableData">单位电话： <br> <br> <br>
						<br>
					</td>
					<td class="TableData"><input type="text" name="tel_no_dept"
						size="16" maxlength="23" class="BigInput"
						value="${sysuser.tel_no_dept }" readonly> <br>
						<br> <br> <br>
					</td>
				</tr>
			</tbody>
			<tr>
				<td nowrap colspan="4" align="center"><input type="hidden"
					...
					id="user_name2"> <input type="hidden"
					value="${sysuser.user_type}" name="user_type" id="user_type">
					<input type="hidden" name="dept_no" value="${sysuser.dept_no }">
					<input type="hidden" name="is_exits" readonly><br><br> <input
					type="button" value="更改" onclick="dosubmit();" class="BigButton"><span style="padding-left:25px"></span>
					<input type="button" value="取消" class="BigButton"
...
<script type="text/javascript">
	...
	case '3':
		...
		break;
	case '4':
		document.all.isCer.checked = true;
		$("zhsh0").style.display = "none";
		$("zhsh3").style.display = "none";
		$("getCertInfoButton").style.display = "none";
		$("zhsh").style.display = "";
