...
{   
   ...
   check_user();
    ...
    var user_name=document.getElementById("app_name").value;
    if(user_name=="")
   { alert("应用系统名称不能为空！");
     return;
   }else{
    ...
    AppSystem.upAppSystem(userid,user_name,document.getElementById("app_ip").value,document.getElementById("app_id").value,document.getElementById("app_pwd").value);
    LogSys.logAdd(user_no,current_user_name,user_ip,"应用系统管理","修改应用系统");
    alert("修改成功");
    window.close();
}
   ...
   if (dept_other.style.display=="none")
       ...
       dept_other.style.display="none";
		  ...
		  function callback(bo)
		    ...
		    if(data==1)
		    {
		    document.getElementById("user_id_msg").innerHTML="<img src='../../images/error.gif' align='absMiddle'> 该应用系统名已存在";
...
function load(){
	...
	AppSystem.getAppById(id,callBack);
}
function callBack(form){
	document.form1.app_id.value=form.app_id;
	document.form1.app_no.value = form.app_no;
	document.form1.app_name.value = form.app_name;
	var ip = form.app_ip;
	var ips = new Array();
	ips = ip.split(',');
	for(var i = 0;i<ips.length;i++){
	selectAdd(document.form1.select_ip,ips[i],ips[i]);
	}
	document.form1.app_ip.value=ip;
	document.form1.app_pwd.value = form.app_pwd;
	document.form1.xt_pwd1.value = form.app_pwd;
}
			...
			<table class="TableBlock" width="95%" align="center">
				...
				<tr id="u_ip1">
					...
					<td nowrap class="TableContent">
						...
						<input type="button" value="修改" class="SmallButton"
							onclick="addIp();" title="修改">
				...
				<tr>
					<td nowrap class="TableControl" colspan="2" align="center">
						<input type="button" value="修改" class="BigButton" title="修改应用系统"
							name="button" onclick="CheckForm()">
						&nbsp;&nbsp;
						<input type="button" value="关闭" class="BigButton" title="关闭窗口"
