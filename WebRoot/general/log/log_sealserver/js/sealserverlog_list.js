...
var cdt_f={};
var logUser="";
var thisUrl=baseUrl(3)+"Seal/general/log/log_sealserver/sealserverlog_list.jsp";
function load(){
//alert(location);
...
function getDatas(){
	...
	LogSys.logSealServerSel(page,pageSize,cdt_f,cb_data); 
}
...
function get_count(){
	...
	LogSys.getCount(starttime,endtime,padding_data);
}
...
//	row.push(d.log_no);
	...
	if(d.file_name.length>25){
		row.push(d.file_name.substring(0,24)+"<input type='button' onclick=\"onclickStr('"+d.file_name+"')\" class='SmallButton' value='...' />");
	}else{
	...
	row.push(toStr(d));
	if(d.server_id=="1"){
		row.push(toreason1(d));
	}else if(d.server_id=="2"){
		row.push(toreason2(d));
	}else{
		row.push("");
	}	
	return row;
}
function onclickStr(filename){
	alert(filename);
}

function toStr(d){
	var str="";
	if(d.seal_status=="1"||d.seal_status=="-6"||d.seal_status=="-7"||d.seal_status=="-9"){
...
function toreason1(d){
	var str="";
	if(d.seal_status=="-1"){
		str+="<a  style=\"color:red\">图片转换异常</a>";		
	}else if(d.seal_status=="-2"){
		str+="<a  style=\"color:red\">服务器中当前无可用印章</a>";
	}else if(d.seal_status=="-3"){
		str+="<a  style=\"color:red\">程序异常</a>";
	}else if(d.seal_status=="-4"){
		str+="<a  style=\"color:red\">签章失败</a>";
	}else if(d.seal_status=="-5"){
		str+="<a  style=\"color:red\">印章未绑定证书</a>";
	}else if(d.seal_status=="-6"){
		str+="<a  style=\"color:red\">CE上传失败</a>";
	}else if(d.seal_status=="-7"){
		str+="<a  style=\"color:red\">图片转换失败</a>";
	}else if(d.seal_status=="-8"){
		str+="<a  style=\"color:red\">文档打开失败</a>";
	}else if(d.seal_status=="1"){
		str+="<a  style=\"color:green\"></a>";
	}else{
		str+="<a  style=\"color:red\">其它异常</a>";
	}
	return str;
}
function toreason2(d){
	var str="";
	if(d.seal_status=="-1"){
