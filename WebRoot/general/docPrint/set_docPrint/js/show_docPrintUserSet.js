var obj=new QerTable("user_list");
var page=1;
var pageSize=10;
var cdt_s="";
var cdt_f={};
var doc_no="";
var thisUrl=baseUrl(3)+"Seal/general/docPrint/set_docPrint/show_docPrintUserSet.jsp";

function load(){
	doc_no=getUrlParam("doc_no")===null?1:getUrlParam("doc_no");
...
function tb_create(){
	$("div_Usertable").innerHTML=obj.HtmlCode();
...
function getDatas(){
	...
	DocPrintService.showUserSet(page,pageSize,doc_no,cdt_f,cb_data);
}
function cb_data(ps){
	if(ps.datas){
		obj.Clean();
		...
		for(var i=0;i<bs.length;i=i+1){
			...
			obj.AddRow(row,bs[i].user_id,i+1);
		}
	}
}
...
function reloadpage(target){
	page=target;
	location=thisUrl+"?doc_no="+doc_no+"&page="+page+"&cdt="+cdt_s;
...
function toArray(d){
	...
	row.push(d.currNum);
	row.push(d.username);	
	row.push(toAStr(d));//构造操作列中内容
	return row;
}

function showUser(sel){
//	alert(sel);
	var url="/Seal/depttree/dept_tree.jsp?req=showUser&ps="+sel;
	var h=450;
	var w=120;
...
//构造操作列中内容
function toAStr(d){
	var str="新份数：<input type='text' name='newNum"+d.user_id+"' id='newNum"+d.user_id+"' value='' size='3'>";
	str+="<input type='button' value='修改' onclick=updatePrint('";
	str+=d.user_id+"','"+doc_no+"')>";
	return str;
}
//修改打印份数
function updatePrint(user_id,doc_no){
	var res=window.confirm("确认修改？");
	if(!res){
		return;
	}
	var newPrintNum=document.getElementById("newNum"+user_id).value.toLow().Trim();//转换成半角并去除空格
	DocPrintService.updateDocPrintRoleUserbyUser(user_id,doc_no,newPrintNum,updateCallback);
}
