...
function edit_temp(ID){
	location="sealTempEdit.do?temp_id="+ID;
}

function delete_temp(ID){
	if(confirm('确认要删除该印模申请么？')){
		SealTemp.deleteTemp(ID,cts_obj);
	} 
}
function cts_obj(){
  location="tempRegList.do";
}
function checkPage(){
	...
	if (!reg.test(page.value) || eval(page.value) > eval("${pageSplit.totalPage }") || page.value == 0) {
		...
		location="tempRegList.do?pageIndex="+page.value;
	}
}
				...
				<c:forEach var="aa" items="${pageSplit.datas }" varStatus="status">
					<tr class="TableLine1">
					   ...
					   <a href="javascript:;" onClick="show_temp('${aa.temp_data  }');">查看</a>&nbsp;
							...
							<c:if test="${aa.temp_status=='3'}">
								<a href="javascript:;"
									onClick="edit_temp('${aa.temp_id }'); return false;">修改</a>&nbsp;
						<a href="javascript:;"
									onClick="delete_temp('${aa.temp_id }');return false;">删除</a>&nbsp;
						</c:if>
						</td>
					</tr>
				...
				<tr class="TableLine1">
					<td colspan="10" class="pager">
						<div class="pager" align="right">
							...
							<c:if test="${pageSplit.nowPage!=1}">
								<a href="tempRegList.do?pageIndex=1">第一页</a>，
										<a href="tempRegList.do?pageIndex=${pageSplit.nowPage-1}">上一页</a>，
									</c:if>
							<c:if test="${pageSplit.nowPage==pageSplit.totalPage}">
										下一页，最后一页
									</c:if>
							<c:if test="${pageSplit.nowPage!=pageSplit.totalPage}">
								<a href="tempRegList.do?pageIndex=${pageSplit.nowPage+1}">下一页</a>，
										<a href="tempRegList.do?pageIndex=${pageSplit.totalPage}">最后一页</a>
							</c:if>
							转到
