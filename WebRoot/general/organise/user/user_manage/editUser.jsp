	...
	function dosubmit() {
		...
		if ((user_psd2 != "") && (user_psd3 != "")) {
			if (user_psd2 == userId) {
				...
				return false;
			} else if (MD5psd2 == "${sysuser.password1md5}"
					|| MD5psd2 == "${sysuser.password2md5}"
					|| MD5psd2 == "${sysuser.user_psd}") {
				alert("新密码与最近使用过的三次密码相同,请重新输入!");
				return false;
			} else if (user_psd2!="${sysuser.user_psd}") {//修改了密码
				if(!user_psd2.match(expr)){
		...
		LogSys.logAdd("${current_user.user_id}", "${current_user.user_name}",
				"${user_ip}", "用户管理", "更新用户:'" + document.form1.user_name.value
						+ "'成功,原名为:" + document.form1.user_name2.value);//logSys.js
...
<body class="bodycolor" topmargin="5"
	...
	<form action="updateSysUser.do?is_app=${type}" method="post"
		...
		<table class="TableBlock" width="95%" align="center">
					...
					<td class="TableData"><select name="user_theme"
						class="BigSelect">
							<c:if test="${sysuser.user_theme==1}">
								<option value="1" selected>默认主题</option>
							...
							<c:if test="${sysuser.user_theme==3}">
								<option value="1">默认主题</option>
								<option value="2">清新怡然</option>
								<option value="3" selected>简洁明亮</option>
								<option value="4">粉红浪漫</option>
								<option value="5">Vista风格</option>
							</c:if>
							...
							<c:if test="${sysuser.user_theme==4}">
								...
								<option value="2">清新怡然</option>
								<option value="3">简洁明亮</option>
								<option value="4" selected>粉红浪漫</option>
								<option value="5">Vista风格</option>
							</c:if>
							...
							<c:if test="${sysuser.user_theme==5}">
								...
								<option value="3">简洁明亮</option>
								<option value="4">粉红浪漫</option>
								<option value="5" selected>Vista风格</option>
							</c:if>
					</select> <br>
					<br>
					<br>
			...
			<tr>
				<td nowrap colspan="4" align="center"><input type="hidden"
					...
					<input type="hidden" name="is_exits" readonly> <input
					type="button" value="更改" onclick="dosubmit();" class="BigButton">
					<!-- <input type="reset" onclick="doreset();" value="重填" class="BigButton"> -->
