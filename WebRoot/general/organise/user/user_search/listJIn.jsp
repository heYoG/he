...
function checkPage(){
	...
	if (!reg.test(page1) || eval(page1) > eval('${pageSplit.totalPage }') || page1.value == 0) {
		...
		location="serUserlist.do?type=flag1&&pageIndex="+page1+"&&dept_no=${dept_no}";
	}
	}
}
function show_sch(){
				...
				<tr class="TableLine1">
					<td colspan="10" class="pager">
						<div class="pager" align="right">
							...
							<c:if test="${pageSplit.nowPage!=1}">
								<a href="serUserlist.do?type=flag1&&pageIndex=1&&dept_no=${dept_no}">第一页</a>，
										<a
									href="serUserlist.do?type=flag1&&pageIndex=${pageSplit.nowPage-1}&&dept_no=${dept_no}">上一页</a>，
									</c:if>
							...
							<c:if test="${pageSplit.nowPage!=pageSplit.totalPage}">
								<a
									href="serUserlist.do?type=flag1&&pageIndex=${pageSplit.nowPage+1}&&dept_no=${dept_no}">下一页</a>，
										<a
									href="serUserlist.do?type=flag1&&pageIndex=${pageSplit.totalPage}&&dept_no=${dept_no}">最后一页</a>
							</c:if>
							转到
