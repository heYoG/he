...
function setField(field_name,field_type,font_type,font_size,font_color,border_type,font_x,font_y,IS_READ_ONLY,TIPSINFO,ITEM_DATA_TYPE)
{
	try{
		...
		if(vRet=="")
		{
			...
			var vRet = obj.InsertNote("PredefineSignAreaOne",argObj.page,field_type,argObj.StartX,argObj.StartY,field_width,field_height);
			if(vRet=="")
			{
				alert("此字段映射已经添加！");
				ShowDialog2('setFrm');
			...
			obj.SetValue("Page"+(argObj.page+1)+".PredefineSignAreaOne",":PROP:BORDER:0");//设置为无边框
			//obj.SetValue("PredefineSignAreaOne",":PROP:BACKCOLOR:255");
			obj.SetValue('PredefineSignAreaOne',':PROP::CLICKPOP:1');
			obj.SetValue('PredefineSignAreaOne',':PROP:ACTIVATE:0');
