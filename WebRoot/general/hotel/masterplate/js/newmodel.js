...
var thisUrl=baseUrl(3)+"Seal/general/hotel/masterplate/newmodel.jsp";
function load(){
	//防止重复提交
	if(location==baseUrl(4)+"eform/newModel.do"){
		location=baseUrl(4)+"general/hotel/masterplate/newmodel.jsp";
	}
...
//	var dept_no=$("dept_no").value;
	var dept_no = "";
	dwr.engine.setAsync(false); //设置方法调用是否同步，false表示同步
	modelFileSrv.showObjBySch(page,pageSize,"0",dept_no,null,null,null,cdt_f,cb_data);
}
function cb_data(ps){
	if(ps.datas){
		obj.Clean();
...
function toArray(d,i){
	...
	row.push(d.approver);
	var opr=strOpr(d,d.approve_status);
...
function searchModel(){
	...
	modelFileSrv.showObjBySch(page,pageSize,"0",dept_no,null,model_name,cdt_f,cb_data);
	hidden("d_search");
...
function strOpr(d,status){
	var str="";
	if(status==2){
		str+="&nbsp;&nbsp;<a href='#' onclick=\"showTpl('";
		str+=d.model_id+"','"+d.model_name+"','"+d.content_data+"','"+d.printoredit+"','"+d.multipart+"','"+d.isflow+"','"+d.page_no+"','"+d.x_position+"','"+d.y_position+"','"+d.area_width+"','"+d.area_height+"','"+d.area_name+"','"+d.dept_no+"','"+d.dept_name+"','"+d.model_xtype+"','"+d.model_ytype+"','"+d.modelchangex+"','"+d.modelchangey+"','"+d.printsize_width+"','"+d.printsize_height+"');\" >修改</a>";
	}
	return str;
}
