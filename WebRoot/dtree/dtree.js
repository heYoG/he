...
//	alert(this.trees[i].toString());
		str+=this.trees[i].toString();
	}
	return str;
};

QerTree.prototype.add = function (id, pid,bid, name, url, title, target, onclick, icon, iconOpen, open){
	//id=change(id);
	//pid=change(pid);
	var index=this.findTreeIndex(pid);
	if(index!=-1){
		this.trees[index].add(id, pid,bid, name, url, title, target, onclick, icon, iconOpen, open);
	}else{
		var tree=new dTree("d"+id,this.id);
		tree.add(id, -1,bid, name, url, title, target, onclick, icon, iconOpen, open);
...
QerTree.prototype.openTos = function (id){
	var index=this.findTreeBidIndex(id);
	if(index!=-1){
		this.trees[0].openTo(index,true,true);
	}
}

QerTree.prototype.findTreeBidIndex = function (id) {
	var index = 0;
	for(var i=0;i<this.trees.length;i=i+1){
		if( (index =this.trees[i].isExistByBid(id)) != -4){
			return index;
		}
	}
	return -1;
...
function Node(id, pid,bid, name, url, title, target, onclick, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.bid = bid;
	this.name = name;
...
dTree.prototype.add = function(id, pid,bid, name, url, title, target, onclick, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid,bid, name, url, title, target, onclick, icon, iconOpen, open);
};
...
dTree.prototype.isExist = function(id) {
	for(var i=0;i<this.aNodes.length;i=i+1){
		//console.log("id::"+this.aNodes[i].obj);
...
dTree.prototype.isExistByBid = function(id) {
	for(var i=0;i<this.aNodes.length;i=i+1){
		if(this.aNodes[i].bid==id){
			return i;
		}
	}
	return -4;
...
dTree.prototype.s = function(id) {
	...
	if (this.selectedNode != id) {
		...
		eNew.className = "selectNode";
		console.log("s" + this.obj + id);
