...
{
   document.getElementById(this.id).innerHTML = this.msg['loading'];
   ...
   document.getElementById(this.id).innerHTML = this.Node2HTML(rootNode);
}

Tree.prototype.ClickNode = function(id)
{
   var container = document.getElementById(this.id + '_container_' + id);
   var expander = document.getElementById(this.id + '_expand_' + id);
...
Tree.prototype.LoadNode = function(id)
{
   ...
   if(html.trim() == "")  //如果下级节点内容为空，则隐藏+-符号图标，置onclick事件为null
   {
      ...
      document.getElementById(this.id + '_link_' + id).onclick = null;
      document.getElementById(this.id + '_div_' + id).removeChild(container);
   }
}
...
Tree.prototype.Node2HTML = function(rootNode)
{
   ...
   for (var i=0;i<nodes.length;i++) //循环显示一级分类
   {
      ...
      if(op_sms == "1")
         html += ' onmouseover="document.getElementById(\'' + this.id + '_span_op_' + id + '\').style.display=\'inline\';" onmouseout="document.getElementById(\'' + this.id + '_span_op_' + id + '\').style.display=\'none\';"';
...
Tree.prototype.redrawNode = function(id,action,text)
{
   ...
   var div = document.getElementById(this.id + '_div_' + id);
   var link = document.getElementById(this.id + '_link_' + id);
   if(id=="0" || !id || !div || !link)
   {
      this.BuildTree();
      return;
