...
function _del(a)
{
  ...
  {
	  var module=document.getElementById("module_"+a);
	  if(module)
...
//	if(_pl&&(_uli||_pnlo))
	{
		...
		{
			if(cols==2)
			   initHead([document.getElementById("col_l"),document.getElementById("col_r")]);
			else
			   initHead([document.getElementById("col_l"),document.getElementById("col_c"),document.getElementById("col_r")]);
			ay=true
		}
	}
}
...
function initHead(a)
{
	...
	for(var i=0;i<colArray.length;i++)
	{
		for(var j=0;j<colArray[i].childNodes.length-1;j++)
		{
			...
			var head_i=document.getElementById(module_i.id+"_head");
			if(!head_i)
				continue;
			
			head_i.module=module_i;
			head_i.onmouseover=function() {var op_i=document.getElementById(this.module.id+"_op");if(op_i) op_i.style.display="inline";}
			head_i.onmouseout =function() {var op_i=document.getElementById(this.module.id+"_op");if(op_i) op_i.style.display="none";}
			
			/*
			var body_i=document.getElementById(module_i.id+"_body");
			if(body_i)
			{
			   var ul_i=document.getElementById(module_i.id+'_ul');
			   if(ul_i.scrollHeight-ul_i.scrollTop>lines_per_page*20)
         {
			      body_i.module=module_i;
			      body_i.onmouseover=function() {var page_i=document.getElementById(this.module.id+"_page");if(page_i) page_i.style.display="inline";}
			      body_i.onmouseout =function() {var page_i=document.getElementById(this.module.id+"_page");if(page_i) page_i.style.display="none";}
			   }
			}*/
		   ...
		   fixTitleWidth(module_i.id)
			...
			var title_i=document.getElementById(module_i.id+"_title");
...
function fixTitleWidth(module_id)
{
	...
	if(document.getElementById(module_id+"_head").scrollWidth - document.getElementById(module_id+"_text").scrollWidth - 120 - 40 > 0)
	   document.getElementById(module_id+"_title").style.width=document.getElementById(module_id+"_head").scrollWidth - document.getElementById(module_id+"_text").scrollWidth - 120 - 40 + "px";
	if(document.getElementById(module_id+"_head").scrollWidth - document.getElementById(module_id+"_text").scrollWidth - 120 - 40 > 0)
	   document.getElementById(module_id+"_title").style.width=document.getElementById(module_id+"_head").scrollWidth - document.getElementById(module_id+"_text").scrollWidth - 120 - 40 + "px";
}
...
{
   document.getElementById("module_"+module_id+"_ul").innerHTML="<div style='text-align:center;'><img src='/images/loading.gif' align='absMiddle'> 加载中，请稍候……</div>";
   var xmlHttpObj=getXMLHttpObj();
   ...
   xmlHttpObj.onreadystatechange=function()
	{
	 	if(xmlHttpObj.readyState==4)
	 	{
	 		if(xmlHttpObj.status==200)
	 		   document.getElementById("module_"+module_id+"_ul").innerHTML=xmlHttpObj.responseText;
	 		else
	 		   document.getElementById("module_"+module_id+"_ul").innerHTML="更新内容错误，代码："+xmlHttpObj.status;
	 	}
	}
   xmlHttpObj.send(null)
}
...
{
   msgBlockHTML = document.getElementById(msgBlock).innerHTML;
   ...
   {
      document.getElementById("overlay").style.width = bb.scrollWidth+"px";
      document.getElementById("overlay").style.height = bb.scrollHeight+"px";
   }
   else
   {
      document.getElementById("overlay").style.width = bb.scrollWidth+"px";
   ...
   document.getElementById(msgBlock).style.left = ((bb.offsetWidth - document.getElementById(msgBlock).offsetWidth)/2)+"px";
   document.getElementById(msgBlock).style.top  = (60 + bb.scrollTop)+"px";
}
function hide_msg(msgBlock)
{
   document.getElementById(msgBlock).innerHTML = msgBlockHTML;
   document.getElementById("overlay").style.display = 'none';
   document.getElementById(msgBlock).style.display = 'none';
}
function NextPage(module_id, page)
{
   var o=document.getElementById('module_'+module_id+'_ul');
   if(!o)   return;
   o.scrollTop+=page*lines_per_page(module_id)*20;
   if(o.scrollTop > 0)
      document.getElementById('module_'+module_id+'_link_pre').className="";
   else
      document.getElementById('module_'+module_id+'_link_pre').className="PageLinkDisable";
   if(o.scrollHeight-o.scrollTop>lines_per_page(module_id)*20)
      document.getElementById('module_'+module_id+'_link_next').className="";
   else
      document.getElementById('module_'+module_id+'_link_next').className="PageLinkDisable";
}
function lines_per_page(module_id)
{
   return Math.floor(parseInt(document.getElementById("module_"+module_id+"_ul").style.height)/20);
}
function CorrectImg()
{
